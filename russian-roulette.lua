...
db = {
    ...
    translations = {
        br = {
            ...
            yourTurn = ", é a sua vez. Você tem 5 segundos. Aperte espaço para atirar. Aperte a tecla M para girar o tambor do revólver e atirar.",
            tookLong = " morreu por demorar demais para atirar.",
            choosePlayers = "Existem mais pessoas na sala do que o jogo suporta. As primeiras \"%d\" pessoas a falarem \"%s\" entrarão no jogo.",
			chamberSpin = " girou o tambor do revólver. Boa sorte.",
            gameEnd = "O jogo terminou!",
            commands = {
				commands = "<J>!<N>tutorial — Mostra como o jogo funciona.",
				tutorial = "Roleta-russa é um jogo de azar onde os participantes colocam uma bala em uma das câmaras de um revólver. O tambor do revólver é girado e fechado, assim a localização da bala é desconhecida. Os participantes apontam o revólver para suas cabeças e atiram, correndo o risco da provável morte caso a bala esteja na câmara engatilhada."
			}
        },
        en = {
            ...
            playerJoined = " joined.",
            playerLeft = " left. got scared lol",
            starting = "The game is starting...",
            started = "The game started. Good luck.",
            playerDied = " died.",
            playerSurvived = " survived. The gun didn't go off.",
            yourTurn = ", it's your turn. You have 5 seconds. Press space to shoot. Press M to spin the revolver cylinder and shoot.",
            tookLong = " died due to tooking too long to shot.",
            choosePlayers = "There are more people in the room than the game supports. The first \"%d\" people to say \"%s\" will enter the game.",
			chamberSpin = " spun the gun chamber. Good luck.",
            gameEnd = "The game has ended!",
            commands = {
				commands = "<J>!<N>tutorial — Teaches how the game works.",
				tutorial = "Russian-Roulette is a chance-game where the participants put only one bullet in one of the revolver chambers. The cylinder is then spun and closed, so the bullet location is unknown. The participants aim the gun to their heads and shoot, taking the risk of dying if the bullet is in the loaded chamber."
			},
        }
    }
}

db.translation = db.translations[tfm.get.room.community] or db.translations.br
...
function generateLorem(n)
    ...
    for word in base:gmatch('%a+') do
        table.insert(baseWords, word:lower())
    end

    for i=1, n do
        word = word .. baseWords[math.random(#baseWords)] .. " "
    end

    word = word:sub(1, 1):upper() .. word:sub(2, -2) .. "."
    return word
end
...
function sendMsg(m, c, n)
	...
	print(colors[c] .. "» <N>" .. m, n)
end

function addPlayer(n) 
	if not db.players[k] then
		db.players[n] = {
			name = n,
			timesSurvived = 0,
			alive = true,
...
function changeTurn()
	...
	while db.players[key].name == db.turn or db.players[key].alive == false do
		key = keyset[math.random(#keyset)]
	end

	db.turn = db.players[key].name
	db.turnKey = key
	db.players[db.turnKey].hasShot = false

	sendMsg(db.players[key].name .. db.translation.yourTurn, "blue")
end

function fireGun(n)
	if n == db.turn then
		db.players[db.turnKey].hasShot = true
		if db.currentChamber == db.bulletChamber then
			tfm.exec.killPlayer(n)
			db.alivePlayers = db.alivePlayers - 1
			db.players[n].alive = false
			sendMsg(n .. db.translation.playerDied, "red")
			db.bulletChamber = math.random(6)
		else
			sendMsg(n .. db.translation.playerSurvived, "blue")
			db.currentChamber = db.currentChamber + 1
		end
	end
end
...
function endGame()
	...
	sendMsg(db.translation.gameEnd, "green")

	if db.playerCount > db.minPlayers then
		db.word = generateLorem(5)
		sendMsg(db.translation.choosePlayers:format(db.minPlayers, db.word), "green")
		db.choosingPlayers = true
	elseif db.playerCount == db.minPlayers then
...
function startGame()
	db.gameStarted = true
	db.choosingPlayers = false
	db.alivePlayers = db.minPlayers

	sendMsg(db.translation.starting, "red")
	...
	sleep.run(function(sleep)
		...
		if db.bulletChamber == 1 then changeTurn() end

		while db.alivePlayers ~= 1 do
			sleep(5000)

			if db.players[db.turnKey].hasShot == false and db.players[db.turnKey].alive == true then
				tfm.exec.killPlayer(db.turn)
				db.players[db.turnKey].alive = false
				db.alivePlayers = db.alivePlayers - 1
				sendMsg(db.turn .. db.translation.tookLong, "red")
			else changeTurn() end
		end
		endGame()
	end)
...
function eventNewPlayer(n)
	db.playerCount = db.playerCount + 1

	if db.playerCount == 1 then tfm.exec.respawnPlayer(n) end

	sendMsg(db.translation.welcome, "green", n)
	sendMsg(n .. db.translation.playerJoined, "blue")

	tfm.exec.addImage("1799fc7574c.png", "?0", -4, 0, n, 0.817, 0.5)
	tfm.exec.addImage("179aaa7dfa1.png", "!7", -4, 0, n, 0.585, 0.345)

	if db.gameStarted then return end

	tfm.exec.respawnPlayer(n)
	addPlayer(n)

	if db.playerCount == db.minPlayers then startGame() end

	system.bindKeyboard(n,32,true)
	system.bindKeyboard(n,77,true)
end

function eventPlayerLeft(n)
	sendMsg(n .. db.translation.playerLeft, "red")
	if db.players[n] then db.players[n].alive = false end
	db.playerCount = db.playerCount - 1
end

function eventPlayerDied(n)
    local bloodImgs = {"15ef8a9af77.png","15ef8aaa9c6.png","15ef8ab01f6.png","15ef8abd01f.png","15ef8ac0c82.png","15ef8ac9b50.png","15ef8accd61.png","15ef8ad03ec.png","15ef8ad2e52.png","15ef8ad5ba3.png","15ef8ad8721.png","15ef8add6f9.png"}
	tfm.exec.addImage(bloodImgs[math.random(#bloodImgs)], "_1", tfm.get.room.playerList[n].x - 50, tfm.get.room.playerList[n].y - 50, nil)
end

function eventChatMessage(n, m)
	if  m == db.word and db.choosingPlayers then
		tfm.exec.respawnPlayer(n)
		addPlayer(n)
...
function eventChatCommand(n, cmd)
	if cmd == "tutorial" then
		sendMsg(db.translation.commands.tutorial, "green", n)
	elseif cmd == "comandos" or cmd == "commands" then
		sendMsg(db.translation.commands.commands, "green", n)
	end
end

function eventKeyboard(n, k)
	if not db.players[n] then return end
	if n == db.turn and db.players[n].hasShot == false then
		if k == 32 then
			fireGun(n)
		elseif k == 77 then
			sleep.run(function(sleep)
				sendMsg(n .. db.translation.chamberSpin, "red")
				sleep(2000)
				fireGun(n)
			end)
		end
